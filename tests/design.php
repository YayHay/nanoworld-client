...
<html>
	<head>
		...
		<script>
			function init() {
				Nano.Render.init(document.getElementById("canvas"));
				Nano.Render.Characters.create("Test", {body: 1, head: 1, eyes: 1, mouth: 1, hair: 1, arms: 1, legs: 1}, [], [125, 125], 1);
			}
			function part(p, n) {
				var x = Nano.Render.Characters.get("Test");
				x.parts[p] = n;
				Nano.Render.characters.Test = x;
			}
			...
			function spawnMany(n) {
				for(var i = 0; i < n; i++) {
					nn++;
					Nano.Render.Characters.create("c" + nn, {hair:(Math.random()>0.5?1:2),eyes:Math.floor(Math.random() * 4)+1}, [], [Math.floor(Math.random() * 250), Math.floor(Math.random() * 250)], (Math.random()>0.5?1:-1));
				}
			}
			
			function fpstest() {
				if(Nano.Render.currFps < 25) return;
				spawnMany(10);
				dance();
				setTimeout(fpstest, 500);
			}
			function dance() {
				for(var name in Nano.Render.Characters.list()) {
					Nano.Render.Characters.faceAway(name);
				}
			}
	...
	<body>
		...
		<button onclick="Nano.Render.Characters.face('Test', -1)">Left</button>
		<button onclick="Nano.Render.Characters.face('Test', 1)">Right</button>
